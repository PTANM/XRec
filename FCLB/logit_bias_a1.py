...
class SafeItemLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids, scores):
        ...
        # ?? Do nothing early ? preserve fluency
        ...
        # ?? Work per batch (still fast)
        for b in range(scores.size(0)):

            logits = scores[b]

            # ?? Top-k safety filter (CRITICAL)
            ...
            # ?? Apply POSITIVE bias (only inside top-k)
            ...
            # ?? Apply NEGATIVE bias (only inside top-k)
